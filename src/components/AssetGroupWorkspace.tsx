...
export const AssetGroupWorkspace: React.FC<AssetGroupWorkspaceProps> = ({
  ...
  const [expandedRatioPreview, setExpandedRatioPreview] = useState<Record<string, RatioImageKey | null>>({});
                        ...
                        {activeTab === 'background' && (() => {
                          ...
                          return (
                            <div className="mt-2 px-1">
                              ...
                              {expandedKey && (() => {
                                const rd = RATIO_DEFS.find((r) => r.key === expandedKey)!;
                                const ratioImage = item.ratioUrls?.[expandedKey];
                                const displayUrl = ratioImage || item.url;
                                const hasCrop = !!item.ratioCropData?.[expandedKey];
                                    ...
                                    <div className="flex gap-3">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-[10px] text-gray-400 mb-2">
                                          {isCustom
                                            ? 'Custom image assigned'
                                            : hasCrop
                                            ? 'General image with crop'
                                            : 'Using general image'}
                                        </div>
                                            ...
                                            <input
                                              ...
                                              onChange={async (e) => {
                                                ...
                                                if (file) {
                                                  ...
                                                  const folder = assetGroup.folders.background;
                                                  const updated = folder.map((a: AssetItem) =>
                                                    a.id === item.id
                                                      ? { ...a, ratioUrls: { ...(a.ratioUrls || {}), [expandedKey]: dataUrl } }
                                                      : a
                                                  );
                                                  onUpdateAssetGroup({
                                                    ...assetGroup,
                                                    folders: { ...assetGroup.folders, background: updated },
                                                  });
                                                }
                                                e.target.value = '';
                                              }}
                                          ...
                                          {hasCrop && (
                                            <button
                                              onClick={() => {
                                                ...
                                                const updated = folder.map((a: AssetItem) => {
                                                  if (a.id !== item.id) return a;
                                                  const newCropData = { ...(a.ratioCropData || {}) };
                                                  delete newCropData[expandedKey];
                                                  const newFP = { ...(a.ratioFocalPoints || {}) };
                                                  delete newFP[expandedKey];
                                                  return { ...a, ratioCropData: newCropData, ratioFocalPoints: newFP };
                                                });
                                                onUpdateAssetGroup({
                                                  ...assetGroup,
                                                  folders: { ...assetGroup.folders, background: updated },
                                              ...
                                              className="px-2 py-1 rounded bg-white hover:bg-red-50 text-red-400 border border-gray-200 hover:border-red-300 text-[10px] font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                                            >
